	...
	public function Annotation($x, $y, $w, $h, $text, $opt=array('Subtype'=>'Text'), $spaces=0) {
		...
		if (!$this->pdfa_mode || ($this->pdfa_mode && $this->pdfa_version == 3)) {
			if ((($opt['Subtype'] == 'FileAttachment') OR ($opt['Subtype'] == 'Sound')) AND (!TCPDF_STATIC::empty_string($opt['FS']))
				...
				$afrel = 'Source';
				if (!empty($opt['AFRelationship'])) {
					$afrel = $opt['AFRelationship'];
				}
				$this->embeddedfiles[basename($opt['FS'])] = array('f' => ++$this->n, 'n' => ++$this->n, 'file' => $opt['FS'], 'afrelationship' => $afrel);
			}
		}
		...
		foreach ($this->embeddedfiles as $filename => $filedata) {
			...
			if ($data !== FALSE) {
				...
				if ($rawsize > 0) {
					...
					$afrel = isset($filedata['afrelationship']) ? $filedata['afrelationship'] : 'Source';
					$out .= ' /AFRelationship /'.$afrel;
					...
					if ($info['cs'] == 'Indexed') {
						...
						for ($i = 0; $i < $count_info; ++$i) {
							if (($info['trns'][$i] != 0) AND ($info['trns'][$i] != $maxval)) {
								// this is not a binary type mask -: create a SMask
								$trns = '';
								break;
